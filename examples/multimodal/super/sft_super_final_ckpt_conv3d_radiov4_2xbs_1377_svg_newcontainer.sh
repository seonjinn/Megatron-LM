...
#SBATCH --job-name=sft_super_final_ckpt_conv3d_radiov4_2xbs_1377_newcontainer_0422
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME=${MODEL_NAME:-"interactive_sft_super_final_ckpt_conv3d_radiov4_2xbs_1377_newcontainer_${DATETIME}"}
    SPECIAL_TOKENS="--special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box>"
    DEBUG=1
else
    MODEL_NAME=${MODEL_NAME:-"sft_super_final_ckpt_conv3d_radiov4_2xbs_1377_newcontainer_0422"}
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
...
if [[ $DEBUG -eq 1 ]]; then
    ...
    # BZ=128
    BZ=512
    NW=12  # TODO: Revert to 8 for future runs; using 12 so we can drop to 6 to resume w/ 128 nodes
    AD=0.0
    HD=0.0
    LI=5
    EXTRA_ARGS=""
