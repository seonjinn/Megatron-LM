...
#SBATCH --job-name=sft_super_omni_262k_svg_newcontainer_0427

# !! Example launch command !!
# examples/multimodal/launch.sh --name sft_super_omni_262k_svg_newcontainer_0427 --sbatch examples/multimodal/super/sft_super_omni_262k_svg_newcontainer.sh --num-jobs 10
...
USE_BUCKETING=0
USE_CP=1
USE_MEGA_LONG_MODE=0
USE_ULTRA_LONG_MODE=1
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME=${MODEL_NAME:-"interactive_sft_super_omni_262k_svg_newcontainer_${DATETIME}"}
    SPECIAL_TOKENS=" --special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box> <so_embedding> <so_start> <so_end> "
    DEBUG=1
else
    MODEL_NAME=${MODEL_NAME:-"sft_super_omni_262k_svg_newcontainer_0427"}
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
...
# Starting checkpoint: 49k omni SVG output chosen checkpoint
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"/lustre/fsw/portfolios/llmservice/users/${USER}/workspace/output/sft_super_omni_49k_svg_newcontainer_0422_iter_0007110/checkpoints"}
...
DATA_TRAIN=${DATA_TRAIN:-"${SOURCE}/examples/multimodal/super/data_config/sft_mmlongbench_txt_yamls/sft_mmlongbench_txt.yaml"}
...
PACKING_SEQ_LEN=${DECODER_SEQ_LEN}
if [[ $USE_MEGA_LONG_MODE -eq 1 ]]; then
    DECODER_SEQ_LEN=122880
    PACKING_SEQ_LEN=122880
elif [[ $USE_ULTRA_LONG_MODE -eq 1 ]]; then
    PACKING_SEQ_LEN=262144
    DECODER_SEQ_LEN=262144
fi
