...
#SBATCH --job-name=pretrain_super_final_ckpt_radiov4_1377_newcontainer_0422
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="interactive_pretrain_super_final_ckpt_radiov4_1377_newcontainer_${DATETIME}"
    SPECIAL_TOKENS="--special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box>"
    DEBUG=1
else
    MODEL_NAME=${MODEL_NAME:-"pretrain_super_final_ckpt_radiov4_1377_newcontainer_0422"}
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
...
CHECKPOINT_DIR="/lustre/fsw/portfolios/llmservice/users/tpoon/checkpoints/super-final-ckpt-after-mtp-radio-v4-tp2-ep64"
...
if [[ $SLURM_SUBMIT_HOST == *"lbd-lax"* ]]; then
    echo "No 1377 dataset exists for lax, exiting."
    exit 1
