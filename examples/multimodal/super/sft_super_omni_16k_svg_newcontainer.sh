...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="interactive_sft_super_omni_16k_svg_newcontainer_${DATETIME}"
    SPECIAL_TOKENS=" --special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box> <so_embedding> <so_start> <so_end> "
    DEBUG=1
else
    MODEL_NAME="sft_super_omni_16k_svg_newcontainer_0422"
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
...
DATA_TRAIN="${SOURCE}/examples/multimodal/super/data_config/yamls/sft_recipe_13p77.yaml"
