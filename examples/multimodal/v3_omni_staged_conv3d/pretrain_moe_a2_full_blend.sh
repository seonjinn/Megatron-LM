...
#SBATCH --job-name=pretrain_moe_a2_conv3d_0223

# !! Example launch command !!
# examples/multimodal/launch.sh --name pretrain_moe_a2_conv3d_0223 --sbatch examples/multimodal/v3_omni_staged_conv3d/pretrain_moe_a2_full_blend.sh --num-jobs 5
...
MODEL_NAME=${MODEL_NAME:-"pretrain_moe_a2_conv3d_0223"}
DATA_TRAIN=${DATA_TRAIN:-"__CODE_DIR__/examples/multimodal/v3_omni_staged_conv3d/pretrain_moe_a2_full_blend.yaml"}
...
# Starting checkpoint: A1 checkpoint (update this path after training A1)
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"/lustre/fsw/portfolios/llmservice/users/matthieul/workspace/output/pretrain_moe_a1_conv3d_0222/checkpoints"}
...
if [[ $DEBUG -eq 1 || $INTERACTIVE -eq 1 ]]; then
    ...
    EXTRA_ARGS=" --freeze-LM --freeze-ViT"
...
# LM (Mamba block) recompute — full recompute for A2
