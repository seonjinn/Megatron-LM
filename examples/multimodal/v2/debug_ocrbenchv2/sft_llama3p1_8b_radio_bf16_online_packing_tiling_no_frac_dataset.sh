...
#SBATCH --job-name=0_sft_llama3p1_8b_radio_bf16_online_packing_tiling_no_frac_dataset_0702
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="0_sft_llama3p1_8b_radio_bf16_online_packing_tiling_no_frac_dataset_0702"
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
...
if [[ $USE_ONLINE_PACKING -eq 1 ]]; then
    DATA_TRAIN="${SOURCE}/examples/multimodal/v2/data_config/sft_dataset_commercial_v13.16_online_packing_no_frac_dataset.yaml"
else
