...
#SBATCH --job-name=sft_nm_5p5_h_12b_6k_cradio_vlm_v1_rc3_video_13p38_div_4_0801
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="sft_nm_5p5_h_12b_6k_cradio_vlm_v1_rc3_video_13p38_div_4_0801"
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
...
DATA_TRAIN="/lustre/fsw/portfolios/llmservice/users/matthieul/eagle_recipe_online_packing/v13p38_video/v13p38_video_divide_4.0.yaml"
...
if [[ $USE_ONLINE_PACKING -eq 1 ]]; then
    EXTRA_ARGS+=" --packing-buffer-size 4034 --packing-seq-length ${DECODER_SEQ_LEN} --packing-knapsack-algorithm balanced_greedy_knapsack "
fi
