...
#SBATCH --job-name=sft_llama_3p1_8b_radio_vlm_rc3_v13p16_video_stage1_0724
...
VIDEO_MAX_NUM_FRAMES=24     # Values > 0 enable video max num frames.
USE_CP=1

# Remember to update model and job name if running in batch mode!!
if [[ $BATCH -eq 0 ]]; then
    DATETIME=`date +'%y-%m-%d-%H-%M-%S'`
    MODEL_NAME="interactive_sft_llama_3p1_8b_radio_video_stage1_${DATETIME}"
    SPECIAL_TOKENS="--special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box>"
    DEBUG=1
else
    MODEL_NAME="sft_llama_3p1_8b_radio_vlm_rc3_v13p16_video_stage1_0724"
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
