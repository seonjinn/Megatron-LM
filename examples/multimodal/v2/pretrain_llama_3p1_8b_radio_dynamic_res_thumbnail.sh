...
#SBATCH --job-name=pretrain_llama_3p1_8b_cradio_rc3_dynamic_res_thumbnail_commercial_0807
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="interactive_pretrain_llama_3p1_8b_radio_v1_dynamic_res_thumbnail_${DATETIME}"
    SPECIAL_TOKENS="--special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box>"
    DEBUG=1
else
    MODEL_NAME="pretrain_llama_3p1_8b_cradio_rc3_dynamic_res_thumbnail_commercial_0807"
    SPECIAL_TOKENS="--special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\>"
fi
...
if [[ $USE_DYNAMIC_RES -eq 1 ]]; then
    ...
    EXTRA_ARGS+=" --use-thumbnail --thumbnail-area-threshold 0.5 "
fi
