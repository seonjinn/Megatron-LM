...
class NemotronNanoVLV2AudioKwargs(ProcessingKwargs, total=False):
...
class NemotronNanoVLV2ProcessorKwargs(ProcessingKwargs, total=False):
    ...
    audio_kwargs: NemotronNanoVLV2AudioKwargs
    _defaults = {
        "text_kwargs": {
            "padding": False,
        },
...
class NemotronNanoVLV2Processor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs: Unpack[NemotronNanoVLV2ProcessorKwargs],
        ...
        if audio is not None:
            assert len(text) == 1, "Audio is not supported for batch size > 1"
            index = 0
            for i in range(len(text)):
