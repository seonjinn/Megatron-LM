...
def model_provider(
    ...
    """
    args = get_args()
    encoder_pp_size = getattr(args, "encoder_pipeline_model_parallel_size", 0)
    encoder_tp_size = getattr(args, "encoder_tensor_model_parallel_size", 0)

    # Deprecation warning for encoder pipeline parallelism
    if encoder_pp_size > 0 or encoder_tp_size > 0:
        warnings.warn(
    ...
    if vision_model_type.startswith("hf://"):
        assert encoder_tp_size < 2, "Huggingface vision encoders do not support --encoder-tensor-model-parallel-size > 1"
        assert encoder_pp_size == 0, "Huggingface vision encoders do not support --encoder-pipeline-model-parallel-size > 0"
    ...
    if encoder_pp_size > 0:
        assert (
            encoder_pp_size == 1
        ), "vision model and projection can only live on 1 pipeline stage."

        if encoder_tp_size > 0:
            vision_config.tensor_model_parallel_size = encoder_tp_size
    ...
    vision_config.pipeline_model_parallel_size = max(1, encoder_pp_size)
...
def sound_model_provider(base_config, language_hidden_size):
    ...
    if args.allow_missing_sound_model_checkpoint:
        # Use named_parameters/named_buffers instead of state_dict().keys() because
        # checkpoint_wrapper overrides state_dict() to strip the _checkpoint_wrapped_module
        # prefix, but load_state_dict uses the actual module hierarchy which includes it.
        sound_model_param_names = [
            f"sound_model.{name}"
            for name, _ in chain(sound_model.named_parameters(), sound_model.named_buffers())
        ]
        sound_model.register_load_state_dict_post_hook(
    ...
    tokenizer = get_tokenizer()
    sound_token_index = tokenizer.sound_token_index
