...
#SBATCH --job-name=nano35_base3p5_vlm_sft_0722
...
if [[ -n "${NVDATASET_CACHE_DIR}" ]]; then
    ...
    NVDATASET_MOUNTS=(
        "/home/svc-dss"
        "/home/svcdss-aihub-sync"
        "/home/svc-aps-wfc"
    )
    for mount_path in "${NVDATASET_MOUNTS[@]}"; do
        case ",${CONTAINER_MOUNTS}," in
            *,"${mount_path}:${mount_path}",*) ;;
            *) CONTAINER_MOUNTS+=",${mount_path}:${mount_path}" ;;
        esac
    done
fi
...
MODEL_NAME=${MODEL_NAME:-"${EXPERIMENT_ID}_sft"}
...
DATA_TRAIN=${DATA_TRAIN:-"${CODE_DIR}/examples/multimodal/v3_baseline/sft_combined_omni_16k_vlm_only_webbrowse_dss.yaml"}
...
WANDB_RUN_ID=${WANDB_RUN_ID:-"n35e14v2"}
