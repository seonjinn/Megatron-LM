...
#SBATCH --job-name=nano_v35_pretrain_a1_svg_0611

MODEL_NAME=${MODEL_NAME:-nano_v35_pretrain_a1_svg_0611}
...
ALLOW_MISSING_SOUND=1
FREEZE_ARGS="--freeze-LM --freeze-ViT --freeze-sound-model"
STAGE_EXTRA_ARGS=" --recompute-sound "
CHECKPOINT_DIR=${CHECKPOINT_DIR:-"${WORKSPACE}/output/nano_v35_sft_conv3d_radiov4_1377_svg_0611/checkpoints"}
DATA_TRAIN=${DATA_TRAIN:-"${SOURCE}/examples/multimodal/super/data_config/a1_yamls/audio_stage1.yaml"}
BZ=${BZ:-512}
LR=${LR:-1e-3}
MIN_LR=${MIN_LR:-1e-5}
...
if [[ "${INCLUDE_VIDEO}" -eq 1 ]]; then
    VIDEO_MAX_NUM_FRAMES=${VIDEO_MAX_NUM_FRAMES:-32}
