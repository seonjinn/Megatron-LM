...
#SBATCH --job-name=pretrain_moe_radio_parakeet_small_1105
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="interactive_pretrain_moe_radio_parakeet_small_${DATETIME}"
    SPECIAL_TOKENS=" --special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box> <so_embedding> <so_start> <so_end> "
    DEBUG=1
else
    MODEL_NAME="pretrain_moe_radio_parakeet_small_1105"
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
