...
#SBATCH --job-name=pretrain_moe_radio_parakeet_a2_on_a1_on_v1
...
if [[ $DEBUG -eq 1 ]]; then
    ...
    INIT_METHOD_STD=0.02 # init method std
fi
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="pretrain_moe_radio_parakeet_a2_on_a1_on_v1_nodes${SLURM_NNODES}-seq${DECODER_SEQ_LEN}-bs${BZ}-lr${LR}-1228"
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
...
# Pretrain a1 on v1 new - checkpoint
CHECKPOINT_DIR=/lustre/fsw/portfolios/llmservice/users/ehosseiniasl/workspace/output/pretrain_moe_radio_parakeet_a1_on_v1_nodes32-seq16384-bs512-lr1e-3-1226/checkpoints
...
# DATA_TRAIN="${SOURCE}/examples/multimodal/avlm/data_config/a12_v1_pretrain.yaml"
# DATA_TRAIN="${SOURCE}/examples/multimodal/avlm/data_config/a1_v1_pretrain.yaml"
DATA_TRAIN="${SOURCE}/examples/multimodal/avlm/data_config/stage1p5_audio_sft_combined.yaml"
