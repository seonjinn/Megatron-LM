...
#SBATCH --job-name=pretrain_moe_radio_parakeet_a1_on_v1
...
# HuggingFace cache directory (pre-download models here to avoid rate limits)
export HF_HOME="/lustre/fsw/portfolios/llmservice/users/${SLURM_JOB_USER}/huggingface/cache"
export HF_HUB_CACHE="/lustre/fsw/portfolios/llmservice/users/${SLURM_JOB_USER}/huggingface/cache/hub"
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="pretrain_moe_radio_parakeet_a1_on_v1_nodes${SLURM_NNODES}-seq${DECODER_SEQ_LEN}-bs${BZ}-lr${LR}-1226"
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
...
DATA_TRAIN="${SOURCE}/examples/multimodal/avlm/data_config/stage1_audio_sft_combined.yaml"
