...
#SBATCH --job-name=stage1_moe_radio_parakeet_1106
...
if [[ $BATCH -eq 0 ]]; then
    ...
    MODEL_NAME="interactive_stage1_moe_radio_parakeet_${DATETIME}"
    SPECIAL_TOKENS=" --special-tokens <image> <img> </img> <quad> </quad> <ref> </ref> <box> </box> <so_embedding> <so_start> <so_end> "
    DEBUG=1
else
    MODEL_NAME="stage1_moe_radio_parakeet_1106"
    SPECIAL_TOKENS=" --special-tokens \<image\> \<img\> \</img\> \<quad\> \</quad\> \<ref\> \</ref\> \<box\> \</box\> \<so_embedding\> \<so_start\> \<so_end\> "
fi
...
CHECKPOINT_DIR="/lustre/fsw/portfolios/llmservice/users/trintamaki/workspace/output/sft_video_moe_1108/checkpoints"
