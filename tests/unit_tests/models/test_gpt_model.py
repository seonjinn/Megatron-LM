...
def test_preprocess_scatters_padding_mask_on_sequence_parallel_pipeline_stage(
    ...
    scatter_inputs: list[tuple[torch.Tensor, object | None]] = []

    def scatter_to_sequence_parallel_region(
        tensor: torch.Tensor, group: object | None = None
    ) -> torch.Tensor:
    ...
    assert torch.equal(scatter_inputs[0][0], padding_mask.transpose(0, 1))
    assert scatter_inputs[0][0].is_contiguous()
    assert scatter_inputs[0][1] is tp_group
...
def test_preprocess_preserves_padding_mask_without_sequence_parallel(
    ...
    def unexpected_scatter(tensor: torch.Tensor, group: object | None = None) -> torch.Tensor:
...
def test_preprocess_scatters_padding_mask_with_explicit_decoder_input(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    tp_group = object()
    model = SimpleNamespace(
    ...
    decoder_input = torch.zeros(3, 2, 8)
    padding_mask = torch.tensor(
        [[False, True, False, True, False, True], [True, False, True, False, True, False]]
    )
    scatter_inputs: list[tuple[torch.Tensor, object | None]] = []
    ...
    assert preprocessed[0] is decoder_input
    assert len(scatter_inputs) == 1
    assert scatter_inputs[0][1] is tp_group
    assert torch.equal(preprocessed[5], padding_mask[:, :3])
