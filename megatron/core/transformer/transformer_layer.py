...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def _te_cuda_graph_replay(self, *args, **kwargs):
        ...
        For packed sequences, this outer method copies PackedSeqParams tensor fields into the
        captured graph's shared buffers and pops PSP before delegating to
        `_te_cuda_graph_replay_impl`. Multi-bucket / overflow / padded-mismatch cases fall back
        to a non-CG eager forward.
        """
        ...
        fallback_exc = None
        try:
            result = self._te_cuda_graph_replay_impl(args, kwargs, context)
