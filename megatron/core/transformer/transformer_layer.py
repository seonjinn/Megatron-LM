...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def _decompose_packed_seq_params_to_kwargs(kwargs):
        ...
        if cu_seqlens_q is None:
            raise ValueError("THD CUDA graph replay requires cu_seqlens_q.")

        # TE captures a fixed kwarg set. Self-attention may omit the KV fields
        # because Q and KV have identical sequence boundaries; canonicalize
        # them so replay always supplies the four tensor inputs from capture.
        ...
        kwargs['cu_seqlens_q_padded'] = cu_seqlens_q_padded
        kwargs['cu_seqlens_kv_padded'] = cu_seqlens_kv_padded
