...
def _cg_count_emit():
    ...
    if total == 0:
        return
    logging.warning(
        "[CG_COUNT] replay=%d fallback=%d replay_pct=%.2f%% fallback_pct=%.2f%%",
        _CG_REPLAY_COUNT,
        _CG_FALLBACK_COUNT,
        100.0 * _CG_REPLAY_COUNT / total,
        100.0 * _CG_FALLBACK_COUNT / total,
    )


atexit.register(_cg_count_emit)
...
class TransformerLayer(GraphableMegatronModule, BaseTransformerLayer):
    ...
    def get_layer_static_inputs(self, seq_length, micro_batch_size):
        ...
        When packed sequences are enabled, also prepare shared CUDA graph buffers and
        a dummy PackedSeqParams so capture uses the THD attention path.
        ...
        if getattr(self.config, "cuda_graph_packed_seq", False):
            ...
            shared_bufs = PackedSeqParams.get_or_create_shared_cg_buffers(
                seq_length, max_seqs, device, tag="attn"
            )
    ...
    def _te_cuda_graph_replay(self, *args, **kwargs):
        ...
        Packed-sequence metadata is copied into shared static buffers before replay.
        The captured graph itself still sees only tensor inputs.
        """
        fallback_args = args
        fallback_kwargs = dict(kwargs)
        psp = kwargs.get("packed_seq_params")
        if psp is not None and hasattr(self, "_cuda_graph_psp_buffers"):
            bucket_max = self._cuda_graph_psp_buffers["cu_seqlens_q"].shape[0]
        ...
        if (
            ...
            _CG_FALLBACK_COUNT += 1
            return self.forward(*fallback_args, **fallback_kwargs)

        try:
            cuda_graph_output = list(
                super()._te_cuda_graph_replay(*cudagraph_args, **cudagraph_kwargs)
            )
