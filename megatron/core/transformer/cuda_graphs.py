...
class _CudaGraphRunner(torch.nn.Module):
    ...
    def _make_pipeline_output_viewless(self, output):
        ...
        return tree_map(
            lambda value: (
                make_viewless_tensor(inp=value, requires_grad=value.requires_grad, keep_graph=True)
                if torch.is_tensor(value)
                else value
            ),
            output,
...
def _map_te_graphs_to_layers(
    ...
    *,
    ...
    """Map TE's flat capture result into already-installed manager-owned lists."""

    flattened_layers = [layer for layers in callables_per_chunk for layer in layers]
    assert len(flattened_layers) == len(
        owned_graph_lists
    ...
    assert (
        len(graphs) == expected_graph_count
...
class TECudaGraphHelper:
    ...
    def __init__(
        ...
        self._compatibility_bank: TECudaGraphBank | None = None
        self._compatibility_bank_manager: TECudaGraphBankManager | None = None
    ...
    def _get_sample_arguments(
        ...
        chunk_id_list=None,
        *,
        schedule_num_layers_per_chunk=None,
        schedule_num_model_chunks=None,
    ):
        """
        ...
        Args:
            ...
            schedule_num_layers_per_chunk (List[int]): Number of layers represented by each
                schedule chunk. This differs from the canonical model-chunk topology when
                overlap_moe_expert_parallel_comm expands the schedule to one chunk per layer.
            schedule_num_model_chunks (int): Number of chunks represented by ``order``. This
                likewise differs from the canonical model-chunk count for an expanded schedule.
        ...
        if schedule_num_model_chunks is None:
            schedule_num_model_chunks = self.num_model_chunks

        assert (
            len(schedule_num_layers_per_chunk) == schedule_num_model_chunks
        ), "The schedule layer counts must contain one entry per schedule chunk."
        assert sum(schedule_num_layers_per_chunk) == len(
            self.flattened_callables
        ), "The schedule layer counts must cover every graphable layer exactly once."
        assert schedule_num_model_chunks == max(
            order
        ), "schedule_num_model_chunks must match the max chunk id in order."
        if chunk_id_list is None:
            # check only if 1f1b overlap is disabled.
            assert (
                self.num_microbatches == len(order) // schedule_num_model_chunks // 2
        ...
        def _get_layer_static_inputs(layer, model_chunk_idx):
            ...
            if "padding_mask" in static_inputs:
                ...
                previous_signature = getattr(
                    layer, "_te_cuda_graph_padding_mask_signature", padding_mask_signature
                )
                assert (
                    previous_signature == padding_mask_signature
                ), "One TE CUDA graph bank requires one exact per-layer padding-mask surface."
                layer._te_cuda_graph_padding_mask_signature = padding_mask_signature
        ...
        prefix_num_layers = [0]
        for schedule_chunk_idx in range(schedule_num_model_chunks):
            num_layers = schedule_num_layers_per_chunk[schedule_chunk_idx]
            prefix_num_layers.append(prefix_num_layers[-1] + num_layers)

        # Reorganize args and kwargs for input tensor reuse.
        # fwd_sample_queues is keyed by schedule chunk index. The value is a queue of tuples.
        ...
        fwd_idx = [0] * schedule_num_model_chunks
        for idx, chunk_id in enumerate(order):
            schedule_chunk_idx = abs(ceil(chunk_id)) - 1

            if chunk_id > 0:
                if schedule_chunk_idx not in fwd_sample_queues:
                    fwd_sample_queues[schedule_chunk_idx] = []

                sample_start_idx = (
                    prefix_num_layers[schedule_chunk_idx] * self.num_microbatches
                ) + (
                    fwd_idx[schedule_chunk_idx] * schedule_num_layers_per_chunk[schedule_chunk_idx]
                )
                if chunk_id_list is not None:
                    ...
                    model_chunk_idx = schedule_chunk_idx
                ...
                for layer_idx, layer in enumerate(callables_curr_chunk):
                    ...
                    fwd_sample_queues[schedule_chunk_idx].append(
                        (sample_keys, per_callable_fwd_idx)
                    )
                ...
                fwd_idx[schedule_chunk_idx] += 1
            elif ceil(chunk_id) == chunk_id:
                num_consumed_samples = min(
                    len(fwd_sample_queues[schedule_chunk_idx]),
                    schedule_num_layers_per_chunk[schedule_chunk_idx],
                )
                for sample_keys, per_callable_fwd_idx in fwd_sample_queues[schedule_chunk_idx][
                ...
                fwd_sample_queues[schedule_chunk_idx] = fwd_sample_queues[schedule_chunk_idx][
                    num_consumed_samples:
                ]
            else:
    ...
    def _get_cuda_graph_input_data(self, *, num_microbatches: int):
        ...
        chunk_id_list = None
        schedule_num_layers_per_chunk = list(self.num_layers_per_chunk)
        schedule_num_model_chunks = self.num_model_chunks
        if self.config.overlap_moe_expert_parallel_comm:
            ...
            schedule_num_layers_per_chunk = [1] * sum(self.num_layers_per_chunk)
            schedule_num_model_chunks = max(order)
            _order_without_wgrad = []
            for c_id in order:
                if ceil(c_id) != c_id:
                    continue
                _order_without_wgrad.append(c_id)
            self.num_microbatches = len(_order_without_wgrad) // schedule_num_model_chunks // 2
            log_on_each_pipeline_stage(
                logger=logger,
                tp_group=self.tp_group,
                dp_cp_group=self.dp_cp_group,
        ...
        # Generate sample arguments and keyword arguments for capturing.
        sample_args, sample_kwargs = self._get_sample_arguments(
            order,
            chunk_id_list,
            schedule_num_layers_per_chunk=schedule_num_layers_per_chunk,
            schedule_num_model_chunks=schedule_num_model_chunks,
        )
        ...
        def get_make_graphed_callables_kwargs():
            ...
            if is_te_min_version("2.6.0"):
                ...
                # of layers per chunk.
                kwargs['_num_layers_per_chunk'] = schedule_num_layers_per_chunk
    ...
    def _abort_capturing(self):
        """Restore capture globals, offload state, GC, and partial owned lists."""
        ...
        try:
            ...
            cleanup_errors.append(error)
        try:
            resettable = synchronized and is_te_min_version("2.10.0")
        except Exception as error:
            cleanup_errors.append(error)
            resettable = False
        if resettable:
            ...
            for layer in self.flattened_callables:
                for graph in getattr(layer, "cuda_graphs", ()):
                    if id(graph) in reset_ids:
                        continue
                    reset_ids.add(id(graph))
    ...
    def _capture_cuda_graph_lists(self, *, num_microbatches):
        """Capture into the exact empty graph-list identities installed by a manager."""

        try:
            start_time = self._start_capturing()
            for layer in self.flattened_callables:
                assert (
                    not layer.cuda_graphs
            ...
            if not self.flattened_callables:
                ...
                _map_te_graphs_to_layers(
                    ...
                    owned_graph_lists=[layer.cuda_graphs for layer in self.flattened_callables],
    ...
    def create_cuda_graph_bank(
        self, manager: TECudaGraphBankManager, *, num_microbatches: int
    ) -> TECudaGraphBank:
        """Consume this helper to create one manager-owned graph bank."""

        return manager.capture(self, num_microbatches=num_microbatches)

    def create_cudagraphs(self):
        """Capture and activate one bank for the legacy training-loop interface."""
        ...
        bank = self.create_cuda_graph_bank(manager, num_microbatches=requested_count)
        bank.activate()
    ...
    * _finish_capturing wraps captured graphs to filter None values that arise
    ...
    def _get_sample_arguments(
        ...
        *,
        ...
        the pipeline-schedule-aware buffer lifecycle tracking. The schedule-local
        count arguments are accepted for interface compatibility with the parent.
