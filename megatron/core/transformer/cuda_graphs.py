...
def get_te_cuda_graph_replay_stream():
    """Return the side stream used for TE graph replay when overlap is enabled."""
...
def get_te_cuda_graph_replay_event():
    """Return the event used to hand off side-stream replay back to the main stream."""
...
class _RecordEventOnBackward(torch.autograd.Function):
    """No-op forward that records an event at the tail of the backward graph."""
...
def wrap_callable_with_event_record(callable_, event):
    """Wrap a TE-graphable callable so both captured graphs end with event.record()."""
    import functools
    ...
    def wrapped(*args, **kwargs):
        ...
        for i, arg in enumerate(new_args):
            if isinstance(arg, torch.Tensor) and arg.requires_grad:
                new_args[i] = _RecordEventOnBackward.apply(event, arg)
                break
        ...
        if not self.flattened_callables:
            ...
            if getattr(self.config, "cuda_graph_te_overlap_replay", False):
                ...
                callables_to_capture = tuple(
                    wrap_callable_with_event_record(c, event) for c in self.flattened_callables
                )
            else:
                callables_to_capture = tuple(self.flattened_callables)
            with rng_context:
                graphs = make_graphed_callables(
                    callables_to_capture, sample_args, **kwargs
                )
