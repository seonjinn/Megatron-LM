...
class LLaVAModel(MegatronModule):
    ...
    def __init__(
        ...
        self._mtp_enabled = False  # Will be set to True if add_decoder and mtp_block_spec is not None
        ...
        if self.add_decoder:
            ...
            self._mtp_enabled = mtp_block_spec is not None
    ...
    def _preprocess_data(
        ...
        *,
        ...
        with torch.no_grad():
            ...
            if shard_factor is not None and max_seq_len % shard_factor != 0:
                ...
                if os.environ.get("NRL_DEBUG", "0") == "1":
                    print(
                        f"[NRL_CONV3D_DEBUG] llava_model SP/CP padding: "
                        f"max_seq_len {_old_max_seq_len} -> {max_seq_len} "
        ...
        if final_embedding is not None:
            # final_embedding shape: [b, s, h] at this point (before transpose)
            row_norms = final_embedding.norm(dim=-1)  # [b, s]
            zero_norm_mask = row_norms < 1e-6  # [b, s]

            if zero_norm_mask.any():
                num_zeros = zero_norm_mask.sum().item()
                print(f"[LLAVA WARNING] Found {num_zeros} zero-norm embeddings in final_embedding. "
                      f"Please ensure the embedding table does not contain zero embeddings. "
    ...
    def forward(
        ...
        *,
        ...
        if use_inference_kv_cache:
            ...
            if self._dynamic_resolution:
                if context_parallel_lm > 1:
                    ...
                    images, imgs_sizes, vision_packed_seq_params, has_pad_img, num_padded_imgs, local_num_frames = \
                        split_to_context_parallel_ranks_dynamic_res(
                            ...
                            temporal_patch_size=self._video_temporal_patch_size,
                            cp_group=cp_group,
                        )
                ...
                if context_parallel_lm > 1 and images.shape[0] >= 2:
                    cp_images, pad = split_to_context_parallel_ranks(images, cp_group=cp_group)
        ...
        if context_parallel_lm > 1 or self.sequence_parallel_lm:
            loss_weight = None
            if context_parallel_lm > 1 and new_labels is not None:
                acc_lengths = (
