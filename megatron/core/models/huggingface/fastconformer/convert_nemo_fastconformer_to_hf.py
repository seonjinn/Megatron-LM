...
class NeMoFastConformerConverter:
    ...
    def convert(self) -> Dict[str, Any]:
        ...
        feature_extractor = FastConformerFeatureExtractor(**{
            k: v for k, v in feature_extractor_config.items()
...
def main():
    ...
    parser.add_argument("--model_name", type=str, required=True,
                       help="NeMo model name (e.g., nvidia/parakeet-tdt-0.6b-v2)")
    parser.add_argument("--output_dir", type=str, required=True,
                       help="Output directory for HuggingFace model")
    parser.add_argument("--verify", action="store_true",
                       help="Verify conversion by comparing encoder outputs")
    ...
    try:
        ...
        logger.info(f"Encoder params loaded: {conversion_info['encoder_params_loaded']}/{conversion_info['encoder_params_total']}")
