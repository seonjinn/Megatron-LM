...
def _get_text_generation_controller(engine):
    return getattr(engine, "text_generation_controller", getattr(engine, "controller", None))
...
def run_mcore_engine(
    ...
    controller = _get_text_generation_controller(engine)

    if random_seed > 0:
        controller.sampling_rng.manual_seed(random_seed)
...
def parse_avlm_chat_completion_request_from_prompt(prompt: str, tokenizer, sampling_params: SamplingParams, request_id: int) -> AVLMInferenceRequest:
    ...
    assert isinstance(tokenizer, MegatronTokenizerVision), "MultimodalTokenizer is required for chat completion request"
...
def run_mcore_engine_avlm(
    ...
    inference_context = controller.inference_wrapped_model.inference_context
